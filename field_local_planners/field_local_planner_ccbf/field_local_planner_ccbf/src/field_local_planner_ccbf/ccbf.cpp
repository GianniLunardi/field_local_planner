...
Ccbf::Ccbf() : BaseLocalPlanner(), state_(State::UNKNOWN) {
}

void Ccbf::setParameters(const Ccbf::Parameters& p) {
  parameters_ = p;
}

Ccbf::Parameters Ccbf::getParameters() const {
  return parameters_;
}

Twist Ccbf::computeTwist() {
  // Check states
  if (BaseLocalPlanner::distance_to_goal_ > base_parameters_.distance_to_goal_thr) {
    ...
    if (std::fabs(BaseLocalPlanner::heading_towards_goal_) > base_parameters_.orientation_to_goal_thr) {
      ...
      // std::cout << "Ccbf: TURN_TO_GOAL" << std::endl;
      stateTurnToGoal();

    } else {
      // If it's pointing towards the goal, move forward tracking the line
      // std::cout << "Ccbf: FORWARD" << std::endl;
      stateForward();

    }
  } else {
    // std::cout << "Ccbf: TURN_TO_DESTINATION" << std::endl;
...
void Ccbf::stateTurnToDestination() {
